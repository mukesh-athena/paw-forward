...
export default function AdoptModal({ dog, shelterId, shelterName, onClose }) {
      ...
      >
        ...
        {status === "success" ? (
          <div className="p-10 md:p-14 text-center space-y-4">
            ...
            >
              Thank you for choosing {dog.name}. The {shelterName} team will reach
              out to you in a few days to take the next steps.
            </p>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-8 md:p-10">
            {/* Dog preview at top of form */}
            <div className="flex items-center gap-4 mb-8 pb-6 border-b border-[#6B1A1A]/10">
            ...
            <p
              ...
              style={{ fontFamily: "var(--font-montserrat)" }}
            >
              Please fill in your details. The shelter will get in touch with you
            ...
            <div className="space-y-5">
              ...
              <Field label="Contact Number *" name="phone" type="tel" value={formData.phone} onChange={handleChange} required />
              <TextareaField label="Anything you'd like to share?" name="message" value={formData.message} onChange={handleChange} />
            </div>
            ...
            <button
              ...
              className="w-full mt-8 py-4 bg-[#6B1A1A] text-white tracking-wider text-sm uppercase rounded-full hover:bg-[#8a2424] hover:scale-[1.02] transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
              style={{ fontFamily: "var(--font-spartan)" }}
            >
              {status === "submitting" ? "Sending..." : "Submit Adoption Request"}
            </button>
          </form>
        )}
      </div>
